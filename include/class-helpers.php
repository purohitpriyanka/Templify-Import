...
class Helpers {
	...
	public static function data_from_file( $file_path ) {
		...
		if ( ! $data ) {
			return new \WP_Error(
				...
				sprintf(
					__( 'An error occurred while reading a file from your server! Tried reading file from path: %s%s.', 'templify-importer-templates' ),
					'<br>',
					$file_path
				)
			);
	...
	private static function check_wp_filesystem_credentials() {
		...
		$file_system_method = 'direct';
		if ( ! ( $file_system_method === get_filesystem_method() ) ) {
			return new \WP_Error(
				'no_direct_file_access',
				sprintf(
					__( 'This WordPress page does not have %sdirect%s write file access. This plugin needs it in order to save the demo import xml file to the upload directory of your site. You can change this setting with these instructions: %s.', 'templify-importer-templates' ),
					'<strong>',
					'</strong>',
		...
		// 		__( 'An error occurred while retrieving reading/writing permissions to your server (could not retrieve WP filesystem credentials)!', 'templify-importer-templates' )
		// 	);
		// }
		...
		if ( ! WP_Filesystem( $creds ) ) {
			return new \WP_Error(
				...
				__( 'Your WordPress login credentials don\'t allow to use WP_Filesystem!', 'templify-importer-templates' )
			);
		}
	...
	public static function import_data( $raw_data ) {
		...
		if ( empty( $raw_data ) ) {
			return new \WP_Error(
				...
				__( 'Error: Give import data could not be read. Please try a different file.', 'templify-importer-templates' )
			);
		}
		$import_setting = [];
		...
		$results = array(
			'message' => __( 'Give data has been successfully imported.', 'templify-importer-templates' ),
		);
		// Return results.
		return  $results;
	}
	...
	public static function get_log_path() {
		...
		$log_path = $upload_path .  'log_file_' . self::$demo_import_start_time .  '.txt' ;
	...
	public static function register_file_as_media_attachment( $log_path ) {
		...
		$attachment = array(
			...
			'post_title'     =>  esc_html__( 'One Click Demo Import - ', 'templify-importer-templates' )  . preg_replace( '/\.[^.]+$/', '', basename( $log_path ) ),
			'post_content'   => '',
			'post_status'    => 'inherit',
		);
